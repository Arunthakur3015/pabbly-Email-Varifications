...
import { Box, Typography, Button, TextField, InputAdornment, IconButton, Checkbox } from '@mui/material';
import RefreshIcon from '@mui/icons-material/Refresh';

export default function EmailListUI() {
  return (
    <Box sx={{ width: 1086, height: 893.14, backgroundColor: '#fff', borderRadius: 2, overflow: 'hidden' }}>

      {/* Home Section */}
      <Box sx={{ height: 110.44, px: 3, py: 3, display: 'flex', alignItems: 'center' }}>
      ...
      <Box sx={{ height: 48, display: 'flex', alignItems: 'center', px: 3, gap: 2 }}>
        {['All', 'Verified', 'Processing', 'Uploading', 'Unverified'].map((tab, i) => (
          <Button key={i} variant="text" sx={{ fontWeight: 600 }}>{tab}</Button>
        ))}
      </Box>
      ...
      <Box sx={{ height: 94.57, display: 'flex', alignItems: 'center', px: 3, gap: 2 }}>
        <TextField
          ...
          size="small"
        />
        <Button variant="outlined" sx={{ width: 155, height: 48 }}>Select Action</Button>
      ...
      {/* List Row Example */}
      <Box sx={{ height: 103.89, display: 'flex', alignItems: 'flex-start', px: 3, py: 1, bgcolor: '#F5F8FA' }}>
        <Checkbox sx={{ width: 36, height: 36, mt: 1 }} defaultChecked />
        <Box sx={{ width: 357.6, px: 2 }}>
          <Box sx={{ bgcolor: '#F8C775', color: '#000', fontSize: 12, fontWeight: 700, px: 1, py: 0.5, borderRadius: 1, width: 70.46, textAlign: 'center' }}>
            Uploading
          </Box>
          <Typography variant="body1" fontWeight={600} mt={0.5}>List 1</Typography>
        ...
        <Box sx={{ width: 267, px: 2, textAlign: 'right' }}>
          <Button variant="outlined" sx={{ width: 93.99, height: 35.78 }}>Uploading</Button>
        </Box>
      </Box>

      {/* Footer */}
      <Box sx={{ height: 48, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', px: 3 }}>
